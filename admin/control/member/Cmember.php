...
// Query
    $konten = query("SELECT k.ID id, k.NAME name, k.EMAIL email, k.NOHP nohp, k.STATUS_ID statusid, u.NAMA `status` FROM users k JOIN status_user u ON (k.STATUS_ID = u.ID)
                        WHERE k.JENIS_ID = 2 ORDER BY STATUS_ID DESC");
    $jenis = query("SELECT * FROM status_user");
...
// Isset
    if ( isset($_POST["submit"])){
        ...
        echo "
            ...
            document.location.href = 'Cmember.php';
            </script>
        ";
        }else {
        echo "
...
<head>
    ...
    <title>Control Member - SIKOMEDI</title>
            ...
            <!-- Begin Page Content -->
                ...
                <!-- Page Heading -->
                    <div class="d-sm-flex align-items-center mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Member Control</h1>
                    </div>
                    <!-- Content Row -->
                    <div class="row">
                    <!-- Table Summary -->
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <div class="d-flex justify-content-center p-2 pb-2 overflow-y-scroll" style="max-height: 30rem;">
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle"><?= $row["nohp"]?></td>
                                        <td class="align-middle text-center">
                                            <form action="" method="post">
                                                <div class="row d-flex justify-content-center">
                                                    <div class="col-6">
                                                        <input type="text" name="id" hidden value="<?= $row["id"]?>">
                                                        <input type="text" name="email" hidden value="<?= $row["email"]?>">
                                                        <select class="form-select" name="status" id="status">
                                                            <option selected value="<?= $row["statusid"]?>"><?= $row["status"]?></option>
                                                    ...
                                                    <div class="col-3">
                                                        <button type="submit" name="submit" class="btn btn-secondary">Ubah</button>
                                                    </div>
                                                </div>
